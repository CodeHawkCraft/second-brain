...
const Button = ({ variant,type,disabled,classes,onClickWithoutEvent,size='md', text, startIcon, endIcon,onClickWithEvent }: ButtonProps) => {
  return (
    <button type={type} disabled={disabled} onClick={onClickWithEvent?onClickWithEvent:onClickWithoutEvent} 
    className={defaultStyles + " "+ styleSize[size]  + " " + styleVairant[variant] +  " "+classes
